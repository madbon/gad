...
<td id="cell-budget_co-<?= $plan['id'] ?>" class="common-cell-container">
    <p><?= $plan["budget_co"]?></p>
    <div id="actn-btns-budget_co-<?= $plan['id']?>" class="actn-btn-bubble actn-buble-common-class" style="display: none;">
        <button id="btn-select-budget_co-<?= $plan['id']?>" type="button" class="btn btn-info btn-xs btn-select-cell" title="Select" >
            Select
        </button>
    </div>
    ...
    <?php
        $this->registerJs('
            $("#cell-budget_co-'.$plan["id"].'").mouseover(function(){
                var attr_name = "budget_co";
            ...
            $("#btn-select-budget_co-'.$plan["id"].'").click(function(){
                var attr_name = "budget_co";
    ...
    <!-- button edit and textarea -->
        <button id="btn-edit-budget_co-<?= $plan['id']?>" type="button" class="btn btn-primary btn-xs btn-edit-cell" title="Edit"  style="display: none;">
            <span class="glyphicon glyphicon-edit"></span>
        </button>
        <?php
            $this->registerJs('
                $("#btn-edit-budget_co-'.$plan["id"].'").click(function(){
                    var attr_name = "budget_co";
            ...
            ');
        ?>
        <div id="div-edit-budget_co-<?= $plan["id"] ?>" class="bubble div-tooltip-form unik-div-tooltip-form-<?= $plan["id"] ?>">
            <textarea id="txt-edit-budget_co" rows="2" class="form-control tooltip-form"></textarea>
    ...
    <!-- button comment and textarea -->
        <button id="btn-comment-budget_co-<?= $plan['id'] ?>" type="button" class="btn btn-warning btn-xs btn-comment-cell" title="Comment" style="display: none;">
            <span class="glyphicon glyphicon-comment"></span>
        </button>
        <?php
            $this->registerJs('
                $("#btn-comment-budget_co-'.$plan["id"].'").click(function(){
                    var attr_name = "budget_co";
                    $(".div-tooltip-form").hide();
            ...
            ');
        ?>
        <div id="div-comment-budget_co-<?= $plan["id"] ?>" class="bubble-comment div-tooltip-form unik-div-tooltip-form-<?= $plan["id"] ?>">
            <textarea id="txt-comment-budget_co" rows="2" class="form-control tooltip-form" placeholder="Write comment"></textarea>
