...
<td id="cell-performance_target-<?= $plan['id'] ?>" class="common-cell-container">
    <p><?= $plan["performance_target"]?></p>
    <div id="actn-btns-performance_target-<?= $plan['id']?>" class="actn-btn-bubble actn-buble-common-class" style="display: none;">
        <button id="btn-select-performance_target-<?= $plan['id']?>" type="button" class="btn btn-info btn-xs btn-select-cell" title="Select" >
            Select
        </button>
    </div>
    ...
    <?php
        $this->registerJs('
            $("#cell-performance_target-'.$plan["id"].'").mouseover(function(){
                var attr_name = "performance_target";
            ...
            $("#btn-select-performance_target-'.$plan["id"].'").click(function(){
                var attr_name = "performance_target";
    ...
    <!-- button edit and textarea -->
        <button id="btn-edit-performance_target-<?= $plan['id']?>" type="button" class="btn btn-primary btn-xs btn-edit-cell" title="Edit"  style="display: none;">
            <span class="glyphicon glyphicon-edit"></span>
        </button>
        <?php
            $this->registerJs('
                $("#btn-edit-performance_target-'.$plan["id"].'").click(function(){
                    var attr_name = "performance_target";
            ...
            ');
        ?>
        <div id="div-edit-performance_target-<?= $plan["id"] ?>" class="bubble div-tooltip-form unik-div-tooltip-form-<?= $plan["id"] ?>">
            <textarea id="txt-edit-performance_target" rows="2" class="form-control tooltip-form"></textarea>
    ...
    <!-- button comment and textarea -->
        <button id="btn-comment-performance_target-<?= $plan['id'] ?>" type="button" class="btn btn-warning btn-xs btn-comment-cell" title="Comment" style="display: none;">
            <span class="glyphicon glyphicon-comment"></span>
        </button>
        <?php
            $this->registerJs('
                $("#btn-comment-performance_target-'.$plan["id"].'").click(function(){
                    var attr_name = "performance_target";
                    $(".div-tooltip-form").hide();
            ...
            ');
        ?>
        <div id="div-comment-performance_target-<?= $plan["id"] ?>" class="bubble-comment div-tooltip-form unik-div-tooltip-form-<?= $plan["id"] ?>">
            <textarea id="txt-comment-performance_target" rows="2" class="form-control tooltip-form" placeholder="Write comment"></textarea>
