...
<td style="<?= $customStyle ?>" title="<?= $column_title ?>" colspan="<?= $colspanValue ?>" id="cell-<?= $attribute_name ?>-<?= $row_id ?>" class="common-cell-container"> <!-- remove border if no comment  -->
    ...
    <p style="<?= $customStyle ?>" id="content-<?= $attribute_name ?>-<?= $row_id ?>"><?= date('d/m/Y',strtotime($cell_value)); ?>  <!-- Display the content of attribute or cell value -->
    ...
    <!-- button edit and textarea -->
        ...
        <?php
            $this->registerJs('
                $("#btn-edit-'.$attribute_name.'-'.$row_id.'").click(function(){
                    ...
                    $("#txt-edit-'.$attribute_name.'-'.$row_id.'").val("'.(date('d-m-Y',strtotime($cell_value))).'");
        ...
        <div id="div-edit-<?= $attribute_name ?>-<?= $row_id ?>" class="bubble div-tooltip-form unik-div-tooltip-form-<?= $row_id ?>">
            ...
            <div class="div-date" style="padding:5px;">
                <?php
                    echo DatePicker::widget([
                        'id' => "txt-edit-".$attribute_name."-".$row_id,
                        'name' => $attribute_name,
                    ...
                    $("#btn-upd8-'.$attribute_name.'-'.$row_id.'").click(function(){
                        ...
                        $.ajax({
                            ...
                            data: { 
                                ...
                                {
                                    var d8_format = result.replace(/\-/g, "/");
                                    $("#content-"+attr_name+"-'.$row_id.'").text(d8_format);
