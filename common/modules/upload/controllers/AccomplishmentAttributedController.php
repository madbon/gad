...
class AccomplishmentAttributedController extends \yii\web\Controller
{
    public function actionIndex($ruc,$onstep,$tocreate)
    {
        ...
        if (Yii::$app->request->isPost) {
            ...
            if ($model->upload()) {
                ...
                $session['excelDataAccomplismentAttributed'] = $excelDataForUploading;
    ...
    public function actionSaveExcelData($ruc,$onstep,$tocreate)
    {
        ...
        $arr = $session['excelDataAccomplismentAttributed'];
        // echo "<pre>";
        // print_r($arr); exit;
        ...
        foreach ($arr as $key => $val) {
            ...
            $model->controller_id = "gad-accomplishment-report";
            $model->ppa_attributed_program_id = (string)$val[0]; // ppa sectors
            $model->lgu_program_project = $val[1];
            $model->checklist_id = $val[2];
            $model->score_type = $val[3];
            $model->hgdg_pimme = $val[4];
            $model->total_annual_pro_cost = $val[5];
            $model->ar_ap_variance_remarks = $val[6];
    ...
    {
        $path = Yii::getAlias('@webroot').'/uploads/template/excel/Attributed-Programs-AR-Template.xlsx';
        if (file_exists($path)) {
            return Yii::$app->response->sendFile($path);
        }
        else{
            \Yii::$app->getSession()->setFlash('danger', '<strong>Unable to download the template of Attributed Program of Accomplishment Report</strong>');
